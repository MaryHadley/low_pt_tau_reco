...
print (my_weighted_mse)
# myModel class

class myModel():
    def __init__(self, optmizer_index, activation_index, DNN_layers, DNN_neurons, dropout, epochs, n_labels=3): 
...
#        x = Dense(input_shape=self.input_shape)(myInput) # should this actually be Sequential as opposed to Dense??
        ...
        model.compile(optimizer=self.optimizer[self.optimizer_index], 
                      loss=my_weighted_mse, metrics=['acc']) #Change this to my custom loss function when I have written it in 
        return model               
             
    #fit model
    def model_fit(self):
        self.__model.fit_generator(generator=self.generator, validation_data=self.validation_data, epochs=self.epochs, verbose=0,
    ...
    def model_evaluate(self):
        self.model_fit()
        evaluation = self.__model.evaluate_generator(self.validation_data, verbose=0) 
        return evaluation 
...
def run_model(optmizer_index, DNN_layers, activation_index, DNN_neurons, dropout, epochs, labels):
    
    _model = myModel(optmizer_index, DNN_layers, activation_index, DNN_neurons, dropout, epochs, labels)
    
    model_evaluation = _model.model_evaluate()
    return model_evaluation

n_epochs = 2 #Might want to play with this, but let's start here (is there a reason you took this as fixed as opposed to trying to optimize it?)
n_labels = 3
...
bounds = [{'name': 'optmizer_index',        'type': 'discrete',   'domain': (0, 1, 2)},
        ...
        #  {'name': 'batch_size',            'type': 'discrete',   'domain': (2,4)}] #just copied what Thea had, maybe should tweak?
          ]
...
if __name__ == "__main__":
  ...
  opt_model.run_optimization(max_iter=1,report_file='bayOpt.txt') #10000 from Thea
