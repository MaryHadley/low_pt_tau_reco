...
labelPruned = ("genParticles") #for gen sim, different from full on LowPtTauMatcher that has Reco
...
handleMET = Handle ("std::vector<reco::GenMET>") #for gen sim, different from full-on LowPtTauMatcher that has reco stuff
labelMET = ("genMetTrue") #for gen sim, different from full-on LowPtTauMatcher that has reco stuff
...
#            print "I entered here 1"
        ...
        if found_taum and found_taup and len(g_taum_pions) == 3 and len(g_taup_pions) == 3 and found_neu and found_anti_neu:
            ...
            if tag_upsilon: break # we have a good upsilon, so leave the gen upsilon loop
            
            #Do NOT check if there is a neutral pion in the decay chain
...
#         """
    ...
    if tag_upsilon:
        if g_taum_pions[0].eta() > 2.5 or g_taum_pions[0].eta() < -2.5  or g_taum_pions[1].eta() > 2.5 or g_taum_pions[1].eta() < -2.5  or g_taum_pions[2].eta() > 2.5 or g_taum_pions[2].eta() < -2.5  or g_taup_pions[0].eta() > 2.5 or  g_taup_pions[0].eta() < -2.5 or g_taup_pions[1].eta() > 2.5 or g_taup_pions[1].eta() < -2.5 or g_taup_pions[2].eta() > 2.5 or g_taup_pions[2].eta() < -2.5 or g_taum_pions[0].pt() < piPtCut or g_taum_pions[1].pt() < piPtCut or g_taum_pions[2].pt() < piPtCut or  g_taup_pions[0].pt() < piPtCut or  g_taup_pions[1].pt() < piPtCut or g_taup_pions[2].pt() < piPtCut:
